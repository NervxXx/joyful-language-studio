  ...
  return (
    <motion.div className="p-5 md:p-8 lg:p-12 max-w-4xl mx-auto space-y-10" variants={container} initial="hidden" animate="show">
      <motion.div variants={item} className="space-y-1.5">
        <h1 className="text-2xl md:text-3xl font-bold text-foreground font-heading tracking-tight">Pronunciation</h1>
        <p className="text-muted-foreground text-sm">Listen, repeat, and perfect your accent</p>
      </motion.div>
      ...
      <motion.div variants={item} className="space-y-2.5">
        {phrases.map((p) => (
          <Card
            ...
            className={`card-hover cursor-pointer p-4 md:p-5 rounded-xl flex items-center gap-4 transition-all ${
              selected.id === p.id ? "border-primary/30 bg-primary/5 shadow-sm" : "border-border bg-card shadow-sm"
            }`}
          >
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${
              selected.id === p.id ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground"
            }`}>{p.difficulty}</span>
      ...
      <motion.div variants={item}>
        <Card className="p-6 md:p-8 bg-card shadow-sm rounded-xl space-y-6">
          ...
          <div className="flex items-center justify-center gap-5">
            ...
            <button
              ...
              className={`w-16 h-16 rounded-full flex items-center justify-center transition-all shadow-md ${
                recording ? "bg-destructive animate-pulse" : "bg-primary hover:opacity-90"
          ...
          {recording && <p className="text-center text-sm text-destructive font-medium animate-pulse">Listening...</p>}
          {!recording && attempts[selected.id] && (
            <p className="text-center text-sm text-muted-foreground">{attempts[selected.id]} attempt{attempts[selected.id] > 1 ? "s" : ""} recorded</p>
          )}
        </Card>
      </motion.div>
    </motion.div>
