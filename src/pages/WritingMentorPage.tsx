  ...
  return (
    <motion.div className="p-5 md:p-8 lg:p-12 max-w-4xl mx-auto space-y-10" variants={container} initial="hidden" animate="show">
      <motion.div variants={item} className="space-y-1.5">
        <h1 className="text-2xl md:text-3xl font-bold text-foreground font-heading tracking-tight">Writing Mentor</h1>
        <p className="text-muted-foreground text-sm">Practice writing with AI-powered feedback</p>
      </motion.div>
      ...
      <motion.div variants={item}>
        <Card className="p-5 md:p-7 bg-card shadow-sm rounded-xl space-y-5">
          ...
          <Textarea
            ...
            className="min-h-[200px] bg-background border-border rounded-xl resize-none text-base md:text-sm"
          />

          <div className="flex items-center justify-between">
            <span className="text-xs text-muted-foreground font-medium">{text.split(/\s+/).filter(Boolean).length} words</span>
      ...
      {submitted && (
        <motion.div variants={item} initial="hidden" animate="show">
          <Card className="p-5 md:p-7 bg-card shadow-sm rounded-xl space-y-4">
            <h3 className="font-bold text-foreground font-heading text-sm">AI Feedback</h3>
