    ...
    <div className="flex flex-col h-screen bg-background">
      ...
      <header className="flex items-center justify-between px-4 md:px-6 py-3.5 border-b border-border bg-card/80 backdrop-blur-md sticky top-0 z-30">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-bold text-foreground font-heading text-sm">Vocabulary Practice</h1>
        </div>
      ...
      <div className="px-4 md:px-6 py-2.5 border-t border-border bg-card/60 backdrop-blur-sm">
        <p className="text-xs font-semibold text-muted-foreground mb-2">🔤 Active words:</p>
        <div className="flex gap-2 overflow-x-auto pb-1">
          {activeWords.map((word) => (
            <span key={word} className="shrink-0 px-4 py-1.5 rounded-full bg-primary/8 text-primary text-sm font-medium">
              {word}
            </span>
          ))}
        </div>
